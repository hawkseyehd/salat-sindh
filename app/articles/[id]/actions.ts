...
export async function handleLike(contentId: string) {
  ...
  try {
    const result = await toggleReaction(contentId, 'article', session.id, 'like')
...
export async function handleDislike(contentId: string) {
  ...
  try {
    const result = await toggleReaction(contentId, 'article', session.id, 'dislike')
    return { success: result.success, message: result.message }
  } catch (error) {
...
export async function getArticleInteractions(contentId: string) {
  try {
    const [likesDislikes, userReaction] = await Promise.all([
      getLikesDislikes(contentId, 'article'),
      getSession().then(session => 
        session ? getUserReaction(contentId, 'article', session.id) : null
      )
    ])
